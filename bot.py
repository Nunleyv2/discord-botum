...
# DÜZELTME 1: postprocessors kaldırıldı — stream URL'yi doğrudan ffmpeg'e
# verirken postprocess yapmak "format not available" hatasına yol açıyordu.
# format öncelik sırası: opus webm > m4a > en iyi ses > en iyisi
YDL_OPTIONS = {
    ...
    # DÜZELTME 2: yt-dlp'nin kendisi en uygun formatı seçsin
    'prefer_free_formats': True,
    # DÜZELTME 3: age-gate ve cookie sorunlarını azaltır
...
FFMPEG_OPTIONS = {
    ...
    # DÜZELTME 4: bufsize artırıldı, kopmaları azaltır
...
async def play_next(ctx: commands.Context):
    ...
    # DÜZELTME 5: URL değilse arama sorgusuna çevir
    ...
    try:
        with yt_dlp.YoutubeDL(YDL_OPTIONS) as ydl:
            ...
            # Arama sonucu geldiyse ilk entry'yi al
            if "entries" in info:
                info = info["entries"][0]

            # DÜZELTME 6: Güvenilir stream URL seçimi
        ...
        await ctx.send(f"❌ İndirme hatası: {e}\nSonraki şarkıya geçiliyor...")
        await play_next(ctx)
    except Exception as e:
        await ctx.send(f"❌ Çalınamadı: {e}")


def _pick_stream_url(info: dict) -> str:
    """
    Format listesinden en uygun ses URL'sini seç.
    Önce saf ses (vcodec=none) formatlarına bak,
    bulamazsan doğrudan info['url'] kullan.
    """
    formats = info.get("formats", [])

    # Tercih sırası: opus > vorbis > m4a > diğer ses
    preferred_codecs = ["opus", "vorbis", "mp4a"]

    # Sadece ses formatlarını filtrele
    audio_only = [
        f for f in formats
    ...
    if audio_only:
        # Tercih edilen codec varsa onu döndür
        for codec in preferred_codecs:
            for f in reversed(audio_only):
                if codec in (f.get("acodec") or ""):
                    return f["url"]
        # Yoksa en yüksek bitrate'li saf ses
        audio_only.sort(key=lambda f: f.get("abr") or 0)
        return audio_only[-1]["url"]

    # Saf ses yoksa genel URL'ye düş
    return info["url"]
